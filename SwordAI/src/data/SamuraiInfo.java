...
public final class SamuraiInfo {
	 ...
	 */
	  ...
	  public int weapon;
		...
		private int[][][] attackField = ActionInfo.ATTACK_FIELD.SWORD.attack_Field;
...
//	    	System.out.print(direction + " ");
	    	for(int i = 0 ; i < attackField[direction - 1].length ; i++ ){
	    			int[] offset = attackField[direction - 1][i];
	    			int attackX = this.curX + offset[0];
	    			int attackY = this.curY + offset[1];
	    			System.err.println("Occupy X = " + attackX + "  Y = " + attackY);
...
//	    	SamuraiInfo myself = this.samuraiInfo[this.weapon];
	    	...
	    	//攻击范围需要根据武器来定。
	    	//故根据三种武器需要三种不同的方法。以下为SPEAR
	    	int[][] attackField = ActionInfo.ATTACK_FIELD.SWORD.attack_Field[direction];
...
//		}
		...
		private int[] moveStimulation(int direction , int curX , int curY){
			...
			//更新AI的坐标
			curX = curX + offset[direction - 5][0];
			curY = curY + offset[direction - 5][1];
...
//			this.checkThenHide();
	    ...
	    if(this.canMoveOneStep(direction)){
	    	int[] coordinations = this.moveStimulation(direction, curX, curY);
			this.curX = coordinations[0];
			this.curY = coordinations[1];
